...
"""Shared SQLAlchemy filter clauses for public list + random pick.

Single source of truth for r18 / orientation / tags / min_* / exclude ids so
`images_list` does not import private helpers from `random_pick`.
"""
...
def r18_where_clause(*, r18: int, r18_strict: bool) -> object | None:
    r18_i = int(r18)
...
def orientation_where_clause(*, orientation: int | None) -> object | None:
...
def clean_tag_groups(values: Sequence[str] | None) -> list[list[str]]:
    if not values:
        return []
...
def included_tags_where_clause(*, tag_names: Sequence[str]) -> object | None:
    groups = clean_tag_groups(tag_names)
    if not groups:
        return None
...
def excluded_tags_where_clause(*, tag_names: Sequence[str]) -> object | None:
    groups = clean_tag_groups(tag_names)
    if not groups:
        return None
...
def exclude_image_ids_where_clause(*, image_ids: Sequence[int] | None) -> object | None:
    if not image_ids:
        return None
    ids: list[int] = []
    seen: set[int] = set()
...
def clamp_random_key(r: float) -> float:
    value = float(r)
    if value < 0.0:
        return 0.0
    if value >= 1.0:
...
def allowed_int_or_null_clause(column: object, *, allowed: set[int | None]) -> object | None:
...
def build_pick_filter_clauses(
    *,
    ...
    """Shared filter clauses for single/batch ring picks and dual-run counts.
    ...
    exclude_ids_clause = exclude_image_ids_where_clause(image_ids=exclude_image_ids)
    ...
    if ai_type_allowed is not None:
        ...
        if not allowed:
            return None
        clause = allowed_int_or_null_clause(Image.ai_type, allowed=allowed)
    ...
    if illust_type_allowed is not None:
        ...
        if not allowed:
            return None
        clause = allowed_int_or_null_clause(Image.illust_type, allowed=allowed)
        if clause is not None:
            clauses.append(clause)
