...
async def random_image(
    ...
    if format in {"json", "simple_json"} or (format == "image" and redirect == 1):
        ...
        if format == "image" and redirect == 1:
            ...
            ) and image_edge_is_ready(getattr(request.app.state, "settings", None))
            edge_url = (
                resolve_image_edge_redirect_url(
            ...
            if edge_url:
                ...
                if prefer_edge:
                    # Edge ready but sign/path failed → local /i redirect cascade.
    ...
    return await deliver_random_image_stream(
        ...
        settings=request.app.state.settings,
        runtime=runtime,
        httpx_transport=getattr(request.app.state, "httpx_data_transport", None),
        httpx_client=getattr(request.app.state, "httpx_data_client", None),
