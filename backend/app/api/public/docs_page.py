...
{examples["adaptive"]}</code></pre>
    ...
    <section class="card" style="margin-top: 14px;">
      ...
      <p class="muted">Image Edge 诚实字段：<span class="kbd">data.image_edge</span> 含 <span class="kbd">enabled_flag</span> / <span class="kbd">ready</span> / <span class="kbd">base_url_count</span>（与 <span class="kbd">/healthz</span> → <span class="kbd">modules.image_edge</span> 同形；不返回密钥、无外呼探测）。HTML 页顶 chip 同步展示 ready/off。</p>
      <p class="muted">CF API 代理诚实字段：<span class="kbd">data.cf_api_proxy</span> 含 <span class="kbd">enabled_flag</span> / <span class="kbd">ready</span> / <span class="kbd">base_url_count</span>（与 <span class="kbd">/healthz</span> → <span class="kbd">modules.cf_api_proxy</span> 同形；公开面不含 has_secret；无外呼探测）。HTML 页顶 chip 同步展示 ready/off。</p>
      <p class="muted">R2 prewarm 诚实字段：<span class="kbd">data.r2_prewarm</span> 含 <span class="kbd">enabled_flag</span> / <span class="kbd">ready</span> / <span class="kbd">url_configured</span>（ready 需 flag+url+secret，与 <span class="kbd">/healthz</span> → <span class="kbd">modules.r2_prewarm</span> 一致；公开面不暴露 secret 字段；无 webhook 探测）。</p>
      <p class="muted">API Key 限流诚实字段：<span class="kbd">data.api_key_rate_limit</span> 含 <span class="kbd">backend</span> / <span class="kbd">requested</span> / <span class="kbd">redis_url_configured</span> / <span class="kbd">required</span> / <span class="kbd">using_memory_fallback</span>（与 <span class="kbd">/healthz</span> → <span class="kbd">modules.api_key_rate_limit</span> 同形；不返回 Redis URL；无外呼探测）。HTML 页顶 chip 同步展示 required/backend。</p>
      <p class="muted">Job queue 诚实字段：<span class="kbd">data.job_queue</span> 含 <span class="kbd">backend</span> / <span class="kbd">requested</span> / <span class="kbd">implemented</span>（与 <span class="kbd">/healthz</span> → <span class="kbd">modules.job_queue</span> 同形；redis/nats 启动即拒绝）。HTML 页顶 chip 同步展示 backend。</p>
      <p class="muted">Recent dedup 诚实字段：<span class="kbd">data.recent_dedup</span> 含 <span class="kbd">backend</span> / <span class="kbd">requested</span> / <span class="kbd">using_memory_fallback</span>（与 <span class="kbd">/healthz</span> → <span class="kbd">modules.recent_dedup</span> 同形；不返回 Redis URL）。HTML 页顶 chip 同步展示 backend/fallback。</p>
      <p class="muted">方言/工厂标签：<span class="kbd">data.catalog.backend</span> / <span class="kbd">data.tags.backend</span> / <span class="kbd">data.random_service.backend</span> / <span class="kbd">data.random_pick.backend</span>（与 <span class="kbd">/healthz</span> → <span class="kbd">modules.catalog|tags|random_service|random_pick</span> 同形）。HTML 页顶 <span class="kbd">ports:</span> chip 汇总展示。</p>
