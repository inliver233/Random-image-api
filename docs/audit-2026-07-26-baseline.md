...
阶段 1 已闭环(代码侧):B2(`3e784b7`)、B3(`80af2b9`)、H5(`f936aee`)、H6(`9a6ffab`)、M4/M5(`b8aaf1d`)、M6(`d1757e4`)、M7(`ccb37b9`)、M8(`25a53c8`)、M9(`ebb5fa4`)、H7 完整 ETL(`9447c3e`,14 测试全链路;live PG e2e 以 TEST_POSTGRES_URL 门控)。阶段 1 剩余:PG-CUTOVER-1..4(需 Docker)。Live PG 证据(真机迁移/并发 claim/争用/trgm 计划)仍被 Docker daemon 不可用阻塞,测试以 `TEST_POSTGRES_URL` 门控,命令记录于各测试 docstring。**M8 运维影响:线上 SQLite 生产部署在滚动到含 `25a53c8` 的镜像前必须设 `ALLOW_PROD_SQLITE=true`。**
