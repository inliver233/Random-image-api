...
| B7 | in-progress | Runtime CF API pool accepted arbitrary HTTP(S) targets and later sent credentials to them; legacy runtime overlays had no durable proof of deployment. | Immediate trust closure now requires HTTPS `workers.dev` bases, forbids userinfo/IP/localhost/non-443/path/query/fragment values, activates only requested-and-verified runtime bases, and prevents manual enrollment of untrusted bases. Durable account/script/kind identity, authenticated challenge, DNS/IP revalidation, redirect validation, and tamper-safe deployment records remain. | CF registry/admin/proxy/OAuth/hydrate | Registry/overlay/probe tests: 18 passed; Admin/deploy tests: 25 passed; independent review approved the immediate fail-closed boundary. Still required: DNS rebinding/private-address matrix, per-hop redirects, authenticated challenge, durable deployment identity and recovery. | none | partial: `f92381d` |
