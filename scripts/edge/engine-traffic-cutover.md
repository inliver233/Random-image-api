...
Deploy/register of CF workers is **independent** and never flips these flags.
...
| 3. Majority | `50`–`75` | Same; feed batch path ok (`skipped_sticky` on top-up is expected) |
...
| Honesty labels | `skipped_traffic` (pct roll miss), `skipped_sticky` (feed top-up), `skipped_circuit` (open circuit) |
